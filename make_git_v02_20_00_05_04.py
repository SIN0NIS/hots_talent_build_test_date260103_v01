...
<body>
    <div id="header">
        ...
        <div class="search-group">
            <input type="text" id="hero-search" class="search-box" placeholder="영웅 검색 또는 빌드 코드 붙여넣기..." onclick="showList()" oninput="handleSearch(this.value)">
            <button class="load-btn" onclick="loadFromCode()">불러오기</button>
        </div>
    ...
    <div id="footer">
        ...
        <div class="credits">제작자: SINONIS | AI 협조: Gemini</div>
    </div>
    <script>
"""
    ...
    html_script = r"""
                ...
                if (prereq) {
                    if (activeIds.includes(prereq)) {
                        ...
                        if (selectedTalents[tierIdx] === tNum) {
                            ...
                            document.getElementById("desc-"+tierIdx).innerHTML = "특성을 선택하세요.";
                        }
                    }
                }
            });
        ...
        function loadFromCode() {
            ...
            if (!match) return alert("올바른 코드 형식이 아닙니다.");
            ...
            if (!heroEntry) return alert("영웅을 찾을 수 없습니다.");
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(full_html)
    print(f"🎉 6종 핵심 스탯 최적화 버전 완료: '{output_file}' 생성!")
